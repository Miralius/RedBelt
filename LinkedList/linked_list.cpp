...
public:
    ...
    void InsertAfter(Node* node, const T& value) {
        if (node) {
            ...
            node->next = new Node{value, oldNextNode};
        } else {
            PushFront(value);
        }
    }
    ...
    void RemoveAfter(Node* node) {
        if (node) {
            Node* oldNextNode = node->next;
            Node* oldNextNextNode = oldNextNode ? oldNextNode->next : nullptr;
            delete oldNextNode;
            node->next = oldNextNextNode;
        } else {
            PopFront();
        }
    }
    ...
    [[maybe_unused]] [[nodiscard]] const Node* GetHead() const { return head; }
